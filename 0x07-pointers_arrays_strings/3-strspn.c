...
unsigned int _strspn(char *s, char *accept)
{
	...
	for (length = 0; *(s + length) != '\0'; length++)
	{
		...
		{
			/* checks where the first character in string j appears */
			/* in string c and breaks loop */
			if (*(s + length) == *(accept + j))
			{
				break;
			}
		}
		/* checks if there is no string accept to loop through */
		if (!*(accept + j))
			break;
	}
	/* returns length where first character appears */
	return (length);
}
